...
#Function that reads an XML file comprising ensemble quantile-quantile diagrams from 
#the EVS.
readEVSEQQ<-function(file) {
	doc = xmlRoot(xmlInternalTreeParse(file))
	#Read the lead times
	lead.times=as.numeric(unique(unlist(xpathApply(doc, "//lead_hour", xmlValue))))
	...
	for(j in 1:length(lead.times)) {
		...
		node = tmp[2]
		if(!is.null(node)) {
			sampleIndex = xmlSize(node$data)
			interval_node=node$data[sampleIndex]$sampling_intervals[1]$main_interval
			t<-node$data[1]$values
			#For fitted ROC, number of columns varies for fitted
			tst<-c()
			try(tst<-node[2]$data[4]$values)	
			if(!is.null(tst)) t = tst
			t<-unlist(strsplit(xmlValue(t),", "))			
			addMe<-matrix(nrow=3*2,ncol=length(t))
					
			#Main data
			for(k in 1: 2) {
				v<-node$data[k]$values
				l<-interval_node[2]$lower_bound[k]$values
				u<-interval_node[3]$upper_bound[k]$values
				start<-(k-1)*3;
				#Nominal value
      ...
      pairlist(diagram.data=diagrams,lead.times=lead.times)
}
...
#file<-"D:/NOAA_work/HEFS_verification/Reforecast_sensitivity_study/Verification/EVS_outputs/Window_width/DOSC1/DOSC1.Precipitation.GEFS_61D_1x24YR.Mean_continuous_ranked_probability_skill_score.xml"
#d<-readEVSScores(file)
