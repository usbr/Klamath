...
for(iterFile in 1:ctFiles){
	...
	outFile = datFile %>% dplyr::mutate(InitDate = as.Date(InitDate)) %>% dplyr::mutate(date2 = paste0(year(InitDate),month(InitDate),day(InitDate),12)) %>% dplyr::mutate(lead = 24) %>% select(date2,lead,ValueMax ValueMin,Value50,Value10,Value90)
}
