...
nrcsFiles = c('Willimason_Fcast_apr-sep_Volumes.csv','Sprague_Fcast_apr-sep_Volumes.csv')
nrcsNames = c('Will','Sprag')
startDates = c('1993-01-01','1993-01-01')
...
#	outFile = datFile %>% dplyr::mutate(date2 = paste0(format(InitDate, '%m/%d/%Y'),' 12')) %>% dplyr::mutate(lead = ifelse(month(InitDate)==1,2160,ifelse(month(InitDate)==2,1440,ifelse(month(InitDate)==3,720,ifelse(month(InitDate)==4,0,0))))) %>% dplyr::select(date2, lead, ValueMin, Value10, Value50, Value90, ValueMax)
	outFile = datFile %>% dplyr::mutate(date2 = paste0(format(InitDate, '%m/%d/%Y'),' 12')) %>% dplyr::mutate(lead = ifelse(month(InitDate)==1,2160,ifelse(month(InitDate)==2,1440,ifelse(month(InitDate)==3,720,ifelse(month(InitDate)==4,0,0))))) %>% dplyr::select(date2, lead, Value10, Value30, Value50, Value70, Value90)
