...
datPlot1 = datPlot %>% filter(WYear>=1981 & WYear<=1994)

P1 = ggplot(data = datPlot1) +
  geom_line(aes(x = date, y = flow_cfs, colour = Type)) +
  scale_colour_manual(values = c('black', '#B50C00')) +
...
datPlot2 = datPlot %>% filter(WYear>=1995 & WYear<=2008)
...
datPlot = datFlow %>% filter(Location == 'WILLILOC') %>% dplyr::mutate(Lbl = 'Williamson River')
datPlot1 = datPlot %>% filter(WYear>=1981 & WYear<=1994)
